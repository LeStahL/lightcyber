            ...
            for(int _note = _noteL; _note <= _noteU; _note++)
            {
                ...
                if(syn == 119)
                {
                    ...
                    if(drum == 0) { sidechain = min(sidechain, 1. - vel * (clamp(1e4 * Bprog,0.,1.) - pow(Bprog/(L-rel),8.))); }
                        ...
                        amaydrumL = vel*(clamp(2.24*_tri(drop_phase(_t,.06,340.,54.)),-1.,1.)*(1.-smoothstep(-1e-3,0.,_t-.05))+.99*clamp(1.*_tri(drop_phase(_t,.06,340.,54.)+.99*lpnoise(_t,9290.)),-1.,1.)*exp(-29.72*_t)+.08*lpnoise(_t,5755.)*(1.-smoothstep(0.,.91,_t-.67))+.63*lpnoise(_t,3210.)*exp(-_t*2.86)+.88*lpnoise(_t,6454.)*exp(-_t*7.59))*smoothstep(0.,.009,_t);
                        amaydrumR = vel*(clamp(2.24*_tri(drop_phase(_t2,.06,340.,54.)),-1.,1.)*(1.-smoothstep(-1e-3,0.,_t2-.05))+.99*clamp(1.*_tri(drop_phase(_t2,.06,340.,54.)+.99*lpnoise(_t2,9290.)),-1.,1.)*exp(-29.72*_t2)+.08*lpnoise(_t2,5755.)*(1.-smoothstep(0.,.91,_t2-.67))+.63*lpnoise(_t2,3210.)*exp(-_t2*2.86)+.88*lpnoise(_t2,6454.)*exp(-_t2*7.59))*smoothstep(0.,.009,_t2);
                    }
                    else if(drum == 8){
