            ...
            for(int _note = _noteL; _note <= _noteU; _note++)
            {
                ...
                if(syn == 119)
                {
                    ...
                    if(drum == 0) { sidechain = min(sidechain, 1. - vel * (clamp(1e4 * Bprog,0.,1.) - pow(Bprog/(L-rel),8.))); }
                        ...
                        amaydrumL = vel*(clamp(.99*_tri(drop_phase(_t,.05,273.,60.)),-1.,1.)*(1.-smoothstep(-1e-3,0.,_t-.23))+.84*clamp(.8*_tri(drop_phase(_t,.05,273.,60.)+.84*lpnoise(_t,1629.)),-1.,1.)*exp(-25.87*_t)+.02*lpnoise(_t,15859.)*(1.-smoothstep(0.,.92,_t-.23))+.2*lpnoise(_t,7467.)*exp(-_t*24.37)+.12*lpnoise(_t,916.)*exp(-_t*20.62))*smoothstep(0.,.01,_t);
                        amaydrumR = vel*(clamp(.99*_tri(drop_phase(_t2,.05,273.,60.)),-1.,1.)*(1.-smoothstep(-1e-3,0.,_t2-.23))+.84*clamp(.8*_tri(drop_phase(_t2,.05,273.,60.)+.84*lpnoise(_t2,1629.)),-1.,1.)*exp(-25.87*_t2)+.02*lpnoise(_t2,15859.)*(1.-smoothstep(0.,.92,_t2-.23))+.2*lpnoise(_t2,7467.)*exp(-_t2*24.37)+.12*lpnoise(_t2,916.)*exp(-_t2*20.62))*smoothstep(0.,.01,_t2);
                    }
                    else if(drum == 8){
...
env = theta(Bprog)*pow(1.-smoothstep(Boff-rel, Boff, B),2);
                        ...
                        amaysynL = (vel*env_AHDSR(_t,tL,.398,0.,.1,1.,.388)*waveshape(MADD(_t,f,0.,64,1,-.9,(1422.+(2572.*clip((1.+.0297)*_sin((.861-.061*aux)*B)))),12.267,.594,8.74,.008,-.75,0),0.,.1,.5,.2,.5,.6));
                        amaysynR = (vel*env_AHDSR(_t2,tL,.398,0.,.1,1.,.388)*waveshape(MADD(_t2,f,0.,64,1,-.9,(1422.+(2572.*clip((1.+.0297)*_sin((.861-.061*aux)*B)))),12.267,.594,8.74,.008,-.75,0),0.,.1,.5,.2,.5,.6));
