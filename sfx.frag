...
#define NTIME 20
const float pos_B[20] = float[20](0.,2.,8.,10.,11.,12.,31.,34.,40.,42.,46.,50.,51.,52.,62.,64.,70.,71.,72.,95.);
const float pos_t[20] = float[20](0.,4.705882,18.552036,22.996481,25.139338,27.208303,65.208303,71.109943,82.722846,86.722846,94.998708,103.134301,105.134301,107.069785,125.819785,129.819785,141.432688,143.368172,145.243172,187.061354);
const float pos_BPS[19] = float[19](.425,.4333,.45,.4667,.4833,.5,.5083,.5167,.5,.4833,.4917,.5,.5167,.5333,.5,.5167,.5167,.5333,.55);
const float pos_SPB[19] = float[19](2.3529,2.3079,2.2222,2.1427,2.0691,2.,1.9673,1.9354,2.,2.0691,2.0338,2.,1.9354,1.8751,2.,1.9354,1.9354,1.8751,1.8182);
float BPS, SPB, BT;
            ...
            for(int _note = _noteL; _note <= _noteU; _note++)
            {
                ...
                if(syn == 119)
                {
                    ...
                    if(drum == 0) { sidechain = min(sidechain, 1. - vel * (clamp(1e4 * Bprog,0.,1.) - pow(Bprog/(L-rel),8.))); }
                        ...
                        amaydrumL = vel*sinshape(fract(sin(_t*100.*30.)*50000.*20.)*doubleslope(_t,0.,.1,.1)+lpnoise(_t + 0.,6000.),1.,5.)*theta(Bprog)*exp(-2.*Bprog);
                        amaydrumR = vel*sinshape(fract(sin(_t2*100.*30.)*50000.*20.)*doubleslope(_t2,0.,.1,.1)+lpnoise(_t2 + 0.,6000.),1.,5.)*theta(Bprog)*exp(-2.*Bprog);
                    }
                    else if(drum == 6){
                        amaydrumL = vel*(clamp(2.24*_tri(drop_phase(_t,.01,330.,120.)),-1.,1.)*(1.-smoothstep(-1e-3,0.,_t-.26))+.72*clamp(.78*_tri(drop_phase(_t,.01,330.,120.)+.72*lpnoise(_t,1282.)),-1.,1.)*exp(-29.17*_t)+.02*lpnoise(_t,6207.)*(1.-smoothstep(0.,.93,_t-.16))+.73*lpnoise(_t,2799.)*exp(-_t*1.92)+.02*lpnoise(_t,5625.)*exp(-_t*13.04))*smoothstep(0.,.007,_t);
                        amaydrumR = vel*(clamp(2.24*_tri(drop_phase(_t2,.01,330.,120.)),-1.,1.)*(1.-smoothstep(-1e-3,0.,_t2-.26))+.72*clamp(.78*_tri(drop_phase(_t2,.01,330.,120.)+.72*lpnoise(_t2,1282.)),-1.,1.)*exp(-29.17*_t2)+.02*lpnoise(_t2,6207.)*(1.-smoothstep(0.,.93,_t2-.16))+.73*lpnoise(_t2,2799.)*exp(-_t2*1.92)+.02*lpnoise(_t2,5625.)*exp(-_t2*13.04))*smoothstep(0.,.007,_t2);
                    }
                    else if(drum == 8){
